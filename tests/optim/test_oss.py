...
class TestOSSMultipleRanks(TestOSS):
    ...
    def test_collect_shards(self):
        if self.dist_init(self.rank):
            ...
            optimizer_state_dict = optim.utils.broadcast_object(
                optimizer_state_dict, src_rank=reference_rank, group=dist.group.WORLD, dist_device=self.device
            )
    ...
    def test_multiple_groups(self):
        ...
        if self.world_size > 3 and self.dist_init(self.rank):
            sub_group_ranks = [0, 2]
    ...
    def test_state_dict_distributed(self):
        if self.dist_init(self.rank):
            ...
            state_dict2 = optim.utils.broadcast_object(state_dict2, reference_rank, dist.group.WORLD, self.device)
