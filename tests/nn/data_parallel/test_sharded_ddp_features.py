...
def run_one_step(
    rank, world_size, temp_file_name, broadcast_buffers, grad_accumulation, reduce_buffer_size, optimizer_type,
):
...
def test_step(broadcast_buffers, grad_accumulation, reduce_buffer_size, optimizer_type):
    world_size = 2
    temp_file_name = tempfile.mkstemp()[1]

    mp.spawn(
        run_one_step,
        args=(world_size, temp_file_name, broadcast_buffers, grad_accumulation, reduce_buffer_size, optimizer_type,),
        nprocs=world_size,
        join=True,
    )


def run_test_two_inputs(rank, world_size, temp_file_name, reduce_buffer_size):
    device = torch.device("cuda")
    backend = dist.Backend.NCCL
    dist.init_process_group(init_method="file://" + temp_file_name, backend=backend, rank=rank, world_size=world_size)
    torch.cuda.set_device(rank)
...
@pytest.mark.parametrize("reduce_buffer_size", [0, 2 ** 20])
@skip_if_no_cuda
def test_inputs(reduce_buffer_size):
    # Check that the ShardedDDP wrapper accepts tuple(tensors) as inputs
    world_size = 2

    mp.spawn(
        run_test_two_inputs, args=(world_size, tempfile.mkstemp()[1], reduce_buffer_size), nprocs=world_size, join=True,
    )
...
def test_ddp_attributes():
    ...
    # - device_type
    backend = dist.Backend.NCCL
    dist.init_process_group(init_method="file://" + tempfile.mkstemp()[1], backend=backend, rank=0, world_size=1)

    model = Sequential(Linear(2, 3), Linear(3, 3))
    optimizer = OSS(params=model.parameters(), optim=torch.optim.SGD, lr=1e-3, momentum=0.99)
    ddp_model = ShardedDataParallel(model, optimizer, sync_models_at_startup=False)
...
def run_test_device_change(rank, world_size, temp_file_name, reduce_buffer_size):
...
def test_device_change(reduce_buffer_size):
    ...
    temp_file_name = tempfile.mkstemp()[1]
    mp.spawn(
        run_test_device_change, args=(world_size, temp_file_name, reduce_buffer_size), nprocs=world_size, join=True,
    )


def run_test_training_change(rank, world_size, temp_file_name, reduce_buffer_size):
    device = torch.device("cuda")
    backend = dist.Backend.NCCL
...
def test_training_change(reduce_buffer_size):
    ...
    temp_file_name = tempfile.mkstemp()[1]
    mp.spawn(
        run_test_training_change, args=(world_size, temp_file_name, reduce_buffer_size), nprocs=world_size, join=True,
    )


def run_test_ddp_sync_batch_norm(rank, world_size, temp_file_name):
    device = torch.device("cuda")
    backend = dist.Backend.NCCL
    torch.cuda.set_device(rank)
    dist.init_process_group(init_method="file://" + temp_file_name, backend=backend, rank=rank, world_size=world_size)
...
def test_ddp_sync_batch_norm():
    ...
    world_size = 2
    temp_file_name = tempfile.mkstemp()[1]
    mp.spawn(run_test_ddp_sync_batch_norm, args=(world_size, temp_file_name), nprocs=world_size, join=True)


def run_test_two_optimizers(rank, world_size, temp_file_name):
    device = torch.device("cuda")
    backend = dist.Backend.NCCL
    dist.init_process_group(init_method="file://" + temp_file_name, backend=backend, rank=rank, world_size=world_size)
    torch.cuda.set_device(rank)
...
def test_two_optimizers():
    ...
    world_size = 2
    temp_file_name = tempfile.mkstemp()[1]
    mp.spawn(run_test_two_optimizers, args=(world_size, temp_file_name), nprocs=world_size, join=True)


def run_test_gpt2(rank, world_size, temp_file_name):
    INPUT_DIM = 16
    BACH_SIZE = 10
    STEPS = 10
...
def test_gpt2(world_size):
    ...
    temp_file_name = tempfile.mkstemp()[1]
    mp.spawn(run_test_gpt2, args=(world_size, temp_file_name), nprocs=world_size, join=True)


def run_test_multiple_groups(rank, world_size, tempfile_name, reduce_buffer_size):
    device = torch.device("cuda")
    backend = dist.Backend.NCCL
...
@skip_if_less_than_four_gpu
@pytest.mark.parametrize("reduce_buffer_size", [0, 2 ** 20])
def test_multiple_groups(reduce_buffer_size):
    world_size = 4
    temp_file_name = tempfile.mkstemp()[1]

    mp.spawn(
        run_test_multiple_groups, args=(world_size, temp_file_name, reduce_buffer_size), nprocs=world_size, join=True,
    )
