...
class TestGradAcc(DistributedTest):
    ...
    @classmethod
    def _test_transformer(self, rank, group, group_rs, config, use_no_sync_context=True):
    ...
    @classmethod
    def _test_nested_wrapper(self, rank, group, group_rs, config):
...
class TestGradAccCommunication(DistributedTest):
    ...
    @classmethod
    def _test_communication(self, rank, group, group_rs, config, nested_model=False):
        if group.size() == 1:
            return
