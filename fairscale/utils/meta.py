...
####################################################################
# BELOW: TAKEN FROM pytorch_lightning/utilities/meta.py
# Pruned part of the code dependent on Pytorch-lightning
# TODO: Removed if the functionality is released with Pytorch
####################################################################
import importlib
import inspect
import threading
...
def _TORCH_GREATER_EQUAL_1_10():
    return torch.__version__ > '1.10'
...
if _TORCH_GREATER_EQUAL_1_10:
    ...
    def init_meta(*_, **__):
        if not _TORCH_GREATER_EQUAL_1_10:
            return Exception("`init_meta` is supported from PyTorch 1.10.0")
...
def _unset_meta_device(from_created: bool = False) -> None:
    ...
    if not _TORCH_GREATER_EQUAL_1_10:
        raise Exception("`init_meta` is supported from PyTorch 1.10.0")
...
def _set_meta_device() -> None:
    ...
    for subclass in get_all_subclasses(torch.nn.modules.module.Module):
        ...
        if subclass in (Sequential, ModuleList, ModuleDict):
            continue
