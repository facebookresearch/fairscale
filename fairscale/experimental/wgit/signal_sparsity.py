...
def _scatter_topk_to_sparse_tensor(
    ...
    if dim is None and len(top_k_tensor.shape) > 1:
        orig_shape = sparse_tensor.shape
        sparse_tensor = sparse_tensor.reshape(-1)
        _, idx = top_k_tensor.flatten().topk(k)
        sparse_tensor[idx] = to_be_sparsify_tensor.flatten()[idx]
        sparse_tensor = sparse_tensor.reshape(orig_shape)
    else:
...
def _top_k_total_size(tensor: Tensor, topk_dim: Optional[int]) -> int:
    ...
    """
    return tensor.numel() if topk_dim is None else tensor.shape[topk_dim]
...
class SignalSparsity:
    ...
    def _validate_conf(self) -> None:
        ...
        if not (none_or_in_range(self._sst_top_k_percent) and none_or_in_range(self._dst_top_k_percent)):
            raise ValueError("top_k_percent values for sst and dst has to be in the interval (0, 100]")
        ...
        if not (none_or_greater_0(self._sst_top_k_element) and none_or_greater_0(self._dst_top_k_element)):
            raise ValueError("top_k_element values for sst and dst has to be greater than 0.")
